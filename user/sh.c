...
void runcmd(struct cmd *cmd) {
  ...
  case EXEC:
    ...
    error("runcmd: a successful exec should not return");
    break;

  case REDIR:
    rcmd = (struct redircmd *)cmd;
    assert(close(rcmd->fd) == 0);
...
struct cmd *redircmd(struct cmd *subcmd, char *file, 
