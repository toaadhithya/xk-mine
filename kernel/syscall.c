...
int argfd(int n, int *fd) {
  ...
  int arg_fd;
  if (argint(n, &arg_fd) < 0) {
    return -1;
  }

  if (arg_fd < 0 || arg_fd >= NOFILE) {
    return -1;
  }

  if (process->fds[arg_fd] == NULL) {
    return -1;
  }
  
  if (process->fds[arg_fd]->ref_count == 0) {
    return -1;
  }
...
int pipevalidfd(int fd) {
  ...
  // pipe wants to file descriptors at these locations
