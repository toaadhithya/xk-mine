...
void kfree(char *v) {
  ...
  if (r->refcount == 0) {
    ...
    r->user = 0;
    r->va = 0;
    r->refcount = 0; // added this
  }
  if (kmem.use_lock)
      release(&kmem.lock);
}

void increment_refcount(char *v) {
  struct core_map_entry *r;
  if ((uint64_t)v % PGSIZE || v < _end || V2P(v) >= (uint64_t)(npages * PGSIZE)){
...
void check_refcount(char *v) {
  struct core_map_entry *r;
  if ((uint64_t)v % PGSIZE || v < _end || V2P(v) >= (uint64_t)(npages * PGSIZE)){
    panic("check_refcount");
  }
  if (kmem.use_lock)
    acquire(&kmem.lock);

  r = (struct core_map_entry *)pa2page(V2P(v));
  cprintf("refcount is %d\n", r->refcount);
  if (kmem.use_lock)
    release(&kmem.lock);
} 
...
char *kalloc(void) {
  ...
  for (i = 0; i < npages; i++) {
    if (core_map[i].available == 1) {
      ...
      core_map[i].refcount = 1; // Added this
      pages_in_use++;
      free_pages--;
      if (kmem.use_lock)
        release(&kmem.lock);
