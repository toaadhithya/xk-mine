...
static int shallow_copy_vpi_page(struct vpi_page **dst, struct vpi_page *src) {
  int i;
  ...
  for (i = 0; i < VPIPPAGE; i++) {
    ...
    if (srcvpi->used) {
      ...
      if (srcvpi->writable == VPI_WRITABLE || srcvpi->cowpageflag) {
        dstvpi->cowpageflag = 1;
        srcvpi->cowpageflag = 1;
      }
      dstvpi->writable = VPI_READONLY;
      srcvpi->writable = VPI_READONLY;
      dstvpi->ppn = srcvpi->ppn;
      // acquire the kmem lock and increment refcount
      increment_refcount((char *)P2V(srcvpi->ppn << PT_SHIFT));
    }
  }
...
int vspacecowcopy(struct vspace *dst, struct vspace *src) {
  memmove(dst->regions, src->regions, sizeof(struct vregion) * NREGIONS); // copy pointers, make sure this is correct

  struct vregion *vr;
  struct vregion *src_vr = src->regions;
  for (vr = dst->regions; vr < &dst->regions[NREGIONS]; vr++) {
    if (shallow_copy_vpi_page(&vr->pages, src_vr->pages) < 0) {
      return -1;
    }
    src_vr++;
  }
